...
processOneItem idx item = mod ((operations!!idx) item) (11 * 3 * 5 * 7 * 19 * 2 * 13 * 17)
...
processRounds count initItems inspectCounts = if (count >= 10000) then (initItems, inspectCounts) else (processRounds (count + 1) newItems newInspectCounts)
